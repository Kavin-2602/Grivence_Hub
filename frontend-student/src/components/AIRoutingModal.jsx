...
import { X, CheckCircle2, ShieldAlert, Utensils, Bus, Building2, Trophy, GraduationCap, ConciergeBell } from 'lucide-react';
...
export default function AIRoutingModal({ isOpen, onClose, aiResult, onConfirmSubmit, isSubmitting }) {
  ...
  const assignedCode = aiResult.assigned_dept_code || aiResult.department_code || (aiResult.department_name ? aiResult.department_name.toUpperCase() : 'CANTEEN');
  const confidenceScore = aiResult.ai_confidence_score || aiResult.confidence_score || 97;
        ...
        <div className="bg-slate-900 text-white rounded-xl p-4 mb-6 shadow-md border border-slate-800">
          <div className="flex justify-between items-start mb-2">
            <div>
              ...
              <span className="text-base font-bold text-white">{aiResult.category || 'Food / Safety Hazard'}</span>
            </div>
            <div className="text-right">
              <span className="text-[10px] font-extrabold uppercase tracking-widest text-rose-400 block">Priority</span>
              <span className="inline-block px-2.5 py-0.5 bg-rose-500 text-white text-xs font-black rounded-full uppercase tracking-wider">
          ...
          <div className="mt-2 text-xs text-slate-300 italic bg-slate-800/80 p-2.5 rounded-lg border border-slate-700">
            "{aiResult.reasoning || aiResult.ai_routing_reasoning || 'Safety concern detected'}"
          </div>
        </div>
          ...
          {departments.map(dept => {
            ...
            return (
              ...
              >
                ...
                {isAssigned && (
                  <span className="text-[9px] uppercase tracking-wider font-black bg-emerald-600 text-white px-1.5 py-0.2 rounded">Routed</span>
                )}
              </div>
            );
          })}
