...
export default function FeedbackCard({ complaintId, ticketId, isLocked = true, existingFeedback = null }) {
  // All state starts UNSET — no hardcoded stars or tags
  ...
  // If existing feedback was already submitted, display it read-only
  useEffect(() => {
    if (existingFeedback) {
    ...
    // When no existing feedback, state remains at UNSET defaults (0 stars, no tags)
  }, [existingFeedback]);
  ...
  const handleSubmitFeedback = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`http://127.0.0.1:8000/api/v1/complaints/${complaintId}/feedback/`, {
        method: 'POST',
        headers: {
      ...
      if (res.ok) {
        ...
        setErrorMsg(data.error || `Submission failed (HTTP ${res.status}).`);
      }
    } catch (err) {
      setErrorMsg('Network error — could not submit feedback. Please retry.');
    }
  };
      ...
      <form onSubmit={handleSubmitFeedback} className="space-y-4">
        ...
        {!submitted && (
          <div className="flex gap-3 pt-2">
            <button
              type="submit"
              className="flex-1 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs rounded-xl transition shadow flex items-center justify-center gap-1.5 cursor-pointer"
            >
            ...
            {reopenRequested && (
              <button
                type="submit"
                className="py-2.5 px-4 bg-amber-600 hover:bg-amber-700 text-white font-bold text-xs rounded-xl transition shadow flex items-center justify-center gap-1.5 cursor-pointer"
              >
